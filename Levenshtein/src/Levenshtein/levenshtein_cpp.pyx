...
cdef extern from "_levenshtein.hpp":
    cdef vector[uint32_t] beam_greedy(const vector[RF_String]& strings, const int el, const vector[double]& weights, const int b) except +
...
def beamgreedy(strlist, el, wlist = None, b = 1, *):
    weights = extract_weightlist(wlist, len(strlist))
    strings = extract_stringlist(strlist)
    median = beam_greedy(strings, el, weights, b)
