...
static inline std::vector<uint32_t> beam_greedy(const std::vector<RF_String>& strings,
                                                            const int el,
                                                            ...
                                                            const int b)
{
    ...
    std::vector<uint32_t> symlist = {'A','C','G','T'};
    size_t maxlen =
        (size_t)std::max_element(std::begin(strings), std::end(strings), [](const auto& a, const auto& b) {
            return a.length < b.length;
        })->length;
    size_t stoplen = 2 * maxlen + 1;
    auto row = std::vector<size_t>(stoplen + 1);
    struct BeamCandidate {
        std::vector<uint32_t> median;
        double minsum;
        std::vector<double> mediandist; 
        std::vector<std::vector<size_t>> rows;  // Store rows for each string for this candidate
        BeamCandidate(size_t stoplen) {
            median = std::vector<uint32_t>(stoplen);
            mediandist = std::vector<double>(stoplen + 1);
        }
    };
    std::function<bool(const BeamCandidate&, const BeamCandidate&)> compare = 
    [](const BeamCandidate& a, const BeamCandidate& b) {
        return a.minsum > b.minsum; // Max-heap based on distance
    };
    // std::priority_queue<BeamCandidate, std::vector<BeamCandidate>, decltype(compare)> candidates(compare);
    // std::set<BeamCandidate> candidates;
    std::multiset<BeamCandidate, decltype(compare)> candidates(compare);
    BeamCandidate initial(stoplen);
    initial.median = {};
    initial.median.resize(stoplen);
    initial.minsum = 0.0;
    initial.rows.resize(strings.size());
    /* allocate and initialize per-string matrix rows and a common work buffer */
    std::vector<std::vector<size_t>> rows(strings.size());    
    for (size_t i = 0; i < strings.size(); i++) {
        size_t leni = (size_t)strings[i].length;
        initial.rows[i] = std::vector<size_t>(leni + 1);
        std::iota(initial.rows[i].begin(), initial.rows[i].end(), 0);
    }
     ...
     */
    initial.mediandist[0] = 0;
    for (size_t i = 0; i < strings.size(); i++)
        initial.mediandist[0] += strings[i].length + weights[i];
    initial.minsum = initial.mediandist[0];
    candidates.insert(initial);
    ...
    for (size_t len = 1; len <= stoplen; len++) {
        if (len > el) break;
        std::multiset<BeamCandidate, decltype(compare)> next_candidates(compare);
        // for (auto candidate : candidates) {
        for (auto it = candidates.rbegin(); it != candidates.rend(); ++it) {
            auto candidate = *it;
            // done = false;
            // std::cout<<"Currently processing ";
            // for (auto ch: candidate.median) std::cout<<(char)ch;
            // std::cout << '\n';
            std::vector<std::vector<size_t>> rows = candidate.rows;       
            // for (auto r :rows) {
            //     for (auto st: r) std::cout<< st<<' ';
            //     std::cout<<'\n';
            // }
            uint32_t symbol = 0;
            row[0] = len;
            ...
            for (size_t j = 0; j < symlist.size(); j++) {
                BeamCandidate curr_candidate = candidate; 
                // std::cout<<"Trying ";
                // for (auto ch: curr_candidate.median) std::cout<<(char)ch;
                // std::cout<<j<<'\t';
                ...
                for (size_t i = 0; i < strings.size(); i++) 
                    visit(strings[i], [&](auto s1) {
                        size_t* oldrow = curr_candidate.rows[i].data();
                        ...
                        memcpy(oldrow, row.data(), (s1.size() + 1) * sizeof(size_t));
                    });
                /* is this symbol better than all the others? */
                if (next_candidates.size() < b) {
                    curr_candidate.minsum = minsum;
                    curr_candidate.mediandist[len] = totaldist;
                    curr_candidate.median[len - 1] = symbol;
                    // std::cout<<"adding ";
                    // for (auto ch: curr_candidate.median) std::cout<<(char)ch;
                    // std::cout<<'\n';
                    next_candidates.insert(curr_candidate);
                    // std::cout<<"currently having "<<next_candidates.size()<<" candidates\n";
                } else if (next_candidates.begin()->minsum > minsum) {
                    // next_candidates.pop();
                    // std::cout<<"erasing ";
                    // for (auto ch: (*next_candidates.begin()).median) std::cout<<(char)ch;
                    // std::cout<<'\n';
                    next_candidates.erase(next_candidates.begin());
                    curr_candidate.minsum = minsum;
                    curr_candidate.mediandist[len] = totaldist;
                    curr_candidate.median[len - 1] = symbol;
                    // std::cout<<"adding ";
        ...
        if (len == stoplen || (len > maxlen && (*candidates.rbegin()).mediandist[len-1] > (*candidates.rbegin()).mediandist[len - 2])) {
                stoplen = len;
                // done = true;
                break;
        }
        ...
        //     candidates.push_back(next_candidates.top());
        //     next_candidates.pop();
        // }
        
        //For debug
        // std::cout<<len<<"\t Current candidates\n";
        // for (BeamCandidate z: candidates) {
        //     std::cout<<z.minsum<<'\t';
        //     for (uint32_t ch : z.median) {
        //         std::cout << static_cast<char>(ch); // Cast to char for readable output
        //     }
        //     std::cout<<std::endl;
        // }

        // std::cout<<"Last candidate ";
        // for (auto ch: (*candidates.rbegin()).median) std::cout<<(char)ch;
        // for (auto ch: (*candidates.rbegin()).mediandist) std::cout<<ch<< ' ';
        // std::cout<< (*candidates.rbegin()).mediandist
        // if (len == stoplen || (len > maxlen && (*candidates.rbegin()).mediandist[len] > (*candidates.rbegin()).mediandist[len - 1])) {
        //     stoplen = len;
        //     break;
        // }
    }
    ...
    std::vector<uint32_t> best_median;
    for (auto candidate : candidates) {
        auto min_iter = std::min_element(candidate.mediandist.begin(), candidate.mediandist.end());
        if ( *min_iter < best_min) {
            size_t bestlen = 
                std::distance(candidate.mediandist.begin(), min_iter);
            best_median = candidate.median;
            best_median.resize(bestlen-1);
        }
    }
    /* return result */
    return best_median;  
